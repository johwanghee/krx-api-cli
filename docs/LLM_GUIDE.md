...
GitHub Release가 아직 발행되지 않았다면 수동 binary 다운로드나 소스 빌드로 fallback 합니다.
...
- `--install-dir <PATH>`: 설치 경로 override
- `--version <TAG>`: 특정 release tag 고정
...
- 가장 먼저 로컬 준비 상태를 확인하려면 `krx-api-cli doctor`를 사용합니다.
...
- 환경변수 override는 plaintext로 허용되며 암호화 대상이 아닙니다.
- config에 plaintext `auth_key`가 남아 있으면 API 명령은 `plaintext_secret_detected`로 차단됩니다.
- 실제 키 저장/복구 명령:
...
- env var가 있어도 config 파일에 plaintext `auth_key`가 남아 있으면 먼저 seal해야 합니다.
- `config show`의 `profiles.*.auth_key.storage`를 보고 `encrypted/plaintext/absent`를 판단합니다.
...
1. `krx-api-cli doctor --compact`
2. `krx-api-cli catalog summary --compact`
3. 필요한 카테고리 찾기
4. `krx-api-cli <category> --help`
5. 대상 API 선택
6. `krx-api-cli <category> <api> --help`
7. 필수 플래그를 채워 실행
...
- 로컬 준비 상태 점검: `krx-api-cli doctor`
...
- 암호화 상태 확인: `krx-api-cli config key status`
- 기존 평문 config 암호화: `krx-api-cli config seal`
...
- `program_error.plaintext_secret_detected`
  - config에 평문 `auth_key`가 남아 있습니다. `config key status` 후 `config seal`을 우선 실행합니다.
